...
Files on Parse are [limited to 10MB](https://parse.com/docs/rest#files-uploading).
You can upload a file using class method `Post.upload_file`

```ruby
  @post = Post.first()
  result = Post.upload_file(uploaded_file.tempfile, uploaded_file.original_filename, content_type: uploaded_file.content_type)
  @post.thumbnail = {"name" => result["name"], "__type" => "File", "url" => result["url"]}
```

You can also upload a file to a model directly:

```ruby
  def create
    @post = Post.new post_params

    uploaded_file = params.require(:post).slice(:image)['image']
    if @post.upload_file(uploaded_file) && @post.save
...
Uploaded files [should be removed manually](https://parse.com/docs/rest#files-deleting).
You can call `model.delete_file` it you want to delete the file attached
to `image` attribute. You can pass optional parameter for deleting attachment from
different column name `model.delete_file(column: 'photo')`.

```ruby
  def update
    @post = Post.find params[:id]

    @post.delete_file if @post.has_attachment?

    uploaded_file = params.require(:post).slice(:image)['image']
    if @post.upload_file(uploaded_file) && @post.update(post_params)
